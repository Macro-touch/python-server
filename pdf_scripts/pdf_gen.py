...
from pdf_styles import ( 
    table_cell_Style, table_header_style, table_style, TABLE_HEADINGS,
    COL_WIDTH, PRIMARY_HVT_HEADING, PRIMARY_GOV_HEADING, PIE_CHART_HEADING,
    PIE_CHART_TYPE_HEADING, PIE_LINE_HEADING, closure_heading, rb_heading, 
    RANDOM_HEADING, UPI_table_style, closure_table_style 
)
...
class BuildPDF:
    ...
    def __init__(self, table_set1:list, pie_data:list, line_data:list, table_set2:list, closure:list):
        self.PDF_ELEMENTS = []
    ...
    def get_pdf_data(self): return self.PDF_ELEMENTS
    ...
    def create_first_half(self):
        ...
        for sec in list(self.set1):
            ...
            if heading_code == 'HVT': self.PDF_ELEMENTS.append(PRIMARY_HVT_HEADING)
            self.PDF_ELEMENTS.append(heading)

            # Adding table heading
            table_header = sec[1]
            tables.append([Paragraph(str(cell), table_header_style) for cell in table_header])

            # Adding empty row if empty
            if len(sec[1:]) == 1: tables.append([ '-' ] * len(sec[1:][0]))
            ...
            for cells in table_rows:
                tables.append([Paragraph(
                    ...
                    table_header_style if key == 0 and heading_code == 'MOP' else table_cell_Style
    ...
    def create_pie_chart(self):
        ...
        for key, data in enumerate(self.pie_data):
            ...
            if key+1 != len(self.pie_data): self.PDF_ELEMENTS.append(Spacer(1, 100))
    ...
    def create_second_half(self):
        ...
        for sec in list(self.set2):
            ...
            # Adding Primary Heading for HVT, UNUSUAL & DUPLICATE
            if heading_code == 'TDS': self.PDF_ELEMENTS.append(PRIMARY_GOV_HEADING)
            self.PDF_ELEMENTS.append(heading)
            ...
            for cells in table_rows:
                tables.append([Paragraph(
                    (format_functions.format_float(str(cell))), 
                    table_cell_Style) for cell in cells])
    ...
    def create_random_verification(self):
        ...
        closure_table.append([
            ...
            for cell in self.closure[0]
        ])

        # row - 1 
        closure_table.append([
            Paragraph(format_functions.format_float(str(cell)), 
        ...
        # row - 2 
        closure_table.append([
            Paragraph(format_functions.format_float(str(cell), count=True), 
            table_header_style if key == 0 else table_cell_Style
